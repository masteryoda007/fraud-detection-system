...
<style>
    ...
    /* FIXED NAVIGATION STYLES - Simple and reliable */
    .tabs-container {
        background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
        padding: 8px;
        border-radius: 12px;
...
def create_uniform_navigation():
    """Create simple, reliable navigation that actually works"""
    ...
    selected_tab = st.session_state.current_page
    ...
    for idx, (col, page) in enumerate(zip(cols, pages)):
        with col:
            if page == selected_tab:
                # Active tab - styled div (no button needed)
                st.markdown(f"""
                    <div style="
                ...
                # Inactive tab - real button
                if st.button(page, key=f"tab_{idx}", use_container_width=True):
...
def show_contextual_chat_interface():
    ...
    with st.form(key="chat_form", clear_on_submit=True):
        ...
        with col1:
            user_query = st.text_area(
                ...
                placeholder="E.g., 'Based on our previous discussion about night transactions, what specific rules should we implement?'",
                key="chat_input",
                height=80,
    ...
    if analyze_button and user_query:
        ...
        # Clear the chat query from session state
...
def main():
    ...
    # Main navigation - FIXED VERSION
    create_uniform_navigation()
